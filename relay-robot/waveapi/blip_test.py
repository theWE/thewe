...
TEST_BLIP_DATA = {
    ...
    'parentBlipId': None,
    'annotations': [{'range': {'start': 2, 'end': 3}, 'name': 'key', 'value': 'val'}],
...
class TestBlip(unittest.TestCase):
  ...
  def assertBlipStartswith(self, expected, blip):
    actual = blip.text[:len(expected)]
  ...
  def testBlipOperations(self):
    ...
    inlineblip = blip.insert_inline_blip(3)
  ...
  def testBlipRefValue(self):
    ...
    self.assertFalse(blip.first(element.Gadget))
