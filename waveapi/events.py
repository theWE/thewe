...
class DocumentChanged(Event):
  ...
  This event is fired all the type and should be used carefully
  to keep the amount of traffic to the robot reasonable. Use
...
class FormButtonClicked(Event):
  ...
  def __init__(self, json, wavelet):
    ...
    self.button_name = self.properties['button']
...
class GadgetStateChanged(Event):
  ...
  Attributes:
    index: index of the gadget that changed in the document
    old_state: old state of the gadget.
  """
...
class AnnotatedTextChanged(Event):
  ...
  Attributes:
    ...
    value: value of the annoation that changed
  """
...
ALL = [item for item in ALL if is_event(item)]
